...
void Diet::save()
{
    ...
    if(ch3 =='y' || ch3=='Y')
    {
        ...
        cin >> your_name;
        Report.open(string(your_name+"'s BMI Report.txt").c_str()); // the function open doesn't take a variable of type string so .c_str converts it to type const char *
        if (!Report) 
        {
		    cout<< ITALIC << "\n--- File not created! ---"<< CLOSEFORMAT; //file cannot be saved : error
	    }
        else 
        {
    ...
    {
        cout<< ITALIC << "--- File not created! ---"<< CLOSEFORMAT; //user choice
    }
}